...
def update_code(base_path, tree):
    ...
    for name, subtree in tree.items():
        ...
        if isinstance(subtree, dict):
            if "." in name:  # If the name contains a dot, it's a file
                result = code_snippet_extractor(name, input_file)
                # print(f"result : {result}")
                if result:
                    # print(result)
                    with open(path, 'w', encoding="utf-8") as f:
                        f.write(result[name])
                    print(f"{name:30} : Edited")
                else:
                    print(f"{name:30} : Not Edited")
                ...
                # print(f"Directory created: {path}")
                update_code(path, subtree)  # Recursively create subdirectories and files
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_path):
        ...
        print(f"{'#' * 60}")
        print(f"# {'Editted history':^56} #")
        print(f"{'#' * 60}")
        update_code(base_directory, directory_tree) # Create the directory structure
        print(f"{'#' * 60}")
    else:
